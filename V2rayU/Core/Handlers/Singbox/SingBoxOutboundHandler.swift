...
class SingboxOutboundHandler {
    ...
    private func buildSSH() -> SingboxOutbound {
        let sshConfig = profile.entity.getSSHConfig()
        return SingboxOutbound(
            type: "ssh",
            tag: "proxy",
            server: profile.address,
            ...
            user: profile.host.isEmpty ? "root" : profile.host,
            private_key: sshConfig.privateKey.isEmpty ? nil : sshConfig.privateKey,
            private_key_passphrase: sshConfig.privateKeyPassphrase.isEmpty ? nil : sshConfig.privateKeyPassphrase
        )
    }
