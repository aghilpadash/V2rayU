...
final class AppMenuManager: NSObject, NSMenuDelegate {
    ...
    func importFromPasteboard() {
        if let uri = NSPasteboard.general.string(forType: .string), uri.count > 0 {
            ...
            noticeTip(title: "import server fail", informativeText: "no found vmess:// or vless:// or trojan:// or anytls:// or naive:// or naive+https:// or ssh:// or ss:// from Pasteboard")
        }
    }
    ...
    func copyProxyExportCommand() {
        ...
        let httpPort = Int(getEffectiveHttpProxyPort())
        let socksPort = Int(getEffectiveSocksProxyPort())
