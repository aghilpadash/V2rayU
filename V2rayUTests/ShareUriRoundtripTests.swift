...
struct ShareUriRoundtripTests {
    ...
    @Test func sshRoundtrip() throws {
        var original = makeProfile(
            protocol: .ssh, remark: "ssh-rt", address: "ssh.example.com",
            port: 22, password: "sshPassword"
        )
        original.host = "sshUser"
        var sshConfig = ProfileEntity.SSHConfig()
        sshConfig.privateKey = "-----BEGIN OPENSSH PRIVATE KEY-----\n...\n-----END OPENSSH PRIVATE KEY-----"
        sshConfig.privateKeyPassphrase = "sshPassphrase"
        original.setSSHConfig(sshConfig)

        let encoded = SSHUri(from: original).encode()
        #expect(!encoded.isEmpty)
        #expect(encoded.hasPrefix("ssh://"))
        ...
        #expect(result.port == original.port)
        #expect(result.password == original.password)
        #expect(result.host == original.host)
        #expect(result.getSSHConfig().privateKey == sshConfig.privateKey)
        #expect(result.getSSHConfig().privateKeyPassphrase == sshConfig.privateKeyPassphrase)
    }

    // MARK: - ShareUri Factory

    @Test func shareUriFactoryAllProtocols() throws {
        let protocols: [V2rayProtocolOutbound] = [.vmess, .vless, .trojan, .shadowsocks, .hysteria2, .anytls, .naive, .ssh]
