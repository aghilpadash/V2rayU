...
struct ProtocolParserTests {
    ...
    @Test func sshBasicParser() throws {
        let link = "ssh://sshUser:sshPassword@ssh.example.com:22?private_key=dGVzdF9rZXk=&private_key_passphrase=sshPassphrase#my-ssh"
        let uri = SSHUri()
        let url = URL(string: link)!
        let error = uri.parse(url: url)
        ...
        #expect(error == nil)
        let profile = uri.getProfile()
        #expect(profile.protocol == .ssh)
        #expect(profile.host == "sshUser")
        #expect(profile.password == "sshPassword")
        #expect(profile.address == "ssh.example.com")
        #expect(profile.port == 22)
        #expect(profile.getSSHConfig().privateKey == "test_key")
        #expect(profile.getSSHConfig().privateKeyPassphrase == "sshPassphrase")
        #expect(profile.remark == "my-ssh")
    }
    ...
    @Test func importAllProtocolTypes() throws {
        let testCases: [(String, String, V2rayProtocolOutbound)] = [
            ...
            ("ssh://user:pass@e.com:22?private_key=dGVzdF9rZXk=&private_key_passphrase=passphrase#s1", "s1", .ssh),
        ]
