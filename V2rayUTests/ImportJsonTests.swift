    ...
    @Test func importSshFromJson() {
        let json = """
        {
            "outbounds": [{
                ...
                "private_key_passphrase": "sshPassphrase"
            }]
        }
        """
        let profile = importFromJson(json: json)
        ...
        #expect(profile?.host == "root")
        #expect(profile?.password == "sshPassword")
        #expect(profile?.getSSHConfig().privateKey == "-----BEGIN OPENSSH PRIVATE KEY-----\n...\n-----END OPENSSH PRIVATE KEY-----")
